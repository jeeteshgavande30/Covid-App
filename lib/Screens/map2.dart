...
class _MapBubblePageState extends State<MapBubblePage>
  ...
  void initState() {
    ...
    // ];
    for (var i = 0; i < 38; i++) {
      _facebookUsers.add(_UserDetails(g.states[i], g.confirmedcases[i]));
      _twitterUsers.add(_UserDetails(g.states[i], g.recoveredcases[i]));
      _tikTokUsers.add(_UserDetails(g.states[i], g.deathscases[i]));
    }
